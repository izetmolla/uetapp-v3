---
name: generate-rs-creatable-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `rs-creatable`. Use when
  building layout JSON, MCP layout tools, or adding Rs Creatable items to
  LayoutBuilder trees.
---

# Generate Rs Creatable layout JSON
...
```
- [ ] 1. Set type (`rs-creatable`) and unique id
...
## Field guide (primary type: `rs-creatable`)

| Field | Required | Notes |
|-------|----------|-------|
| `type` | yes | `"rs-creatable"` |
| `id` | yes | Unique in tree |
| `name` | yes | Form field key (react-hook-form) |
| `validation` | no | Zod rules object |
| `onLoadFetch` | no | When true (default), fetch on mount when `optionsApi` is set. |
| `onCreateConfigAPI` | no | API config for persisting a newly created option. |
...
2. Use **stable, descriptive ids** (`rs-creatable-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `rs-creatable` |

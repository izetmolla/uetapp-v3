---
name: generate-separator-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `separator`. Use when
  building layout JSON, MCP layout tools, or adding Separator items to
  LayoutBuilder trees.
---

# Generate Separator layout JSON
...
```
- [ ] 1. Set type (`separator`) and unique id
...
## Field guide (primary type: `separator`)
...
2. Use **stable, descriptive ids** (`separator-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `separator` |

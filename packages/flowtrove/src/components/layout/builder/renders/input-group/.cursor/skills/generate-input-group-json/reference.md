# Input Group — JSON reference

TypeScript: `renders/input-group/types.ts`.

Registered manifest types: `input-group`.
...
## InputGroupItem (`type: "input-group"`)

Extends `BaseLayoutItem`.

| Field | Type | Description |
|-------|------|-------------|
| `type` | `"input-group"` | Discriminant |
| `id` | string | Unique identifier |
| `children` | — |  |

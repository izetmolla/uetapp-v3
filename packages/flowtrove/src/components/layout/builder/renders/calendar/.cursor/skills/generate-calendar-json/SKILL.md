...
description: >-
  Generates and validates Flowtrove layout builder JSON for `calendar`. Use when
  building layout JSON, MCP layout tools, or adding Calendar items to
  LayoutBuilder trees.
---

# Generate Calendar layout JSON
...
```
- [ ] 1. Set type (`calendar`) and unique id
...
## Field guide (primary type: `calendar`)
...
2. Use **stable, descriptive ids** (`calendar-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `calendar` |

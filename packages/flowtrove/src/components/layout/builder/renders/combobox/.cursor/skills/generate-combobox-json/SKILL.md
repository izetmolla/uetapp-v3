---
name: generate-combobox-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `combobox`. Use when
  building layout JSON, MCP layout tools, or adding Combobox items to
  LayoutBuilder trees.
---

# Generate Combobox layout JSON
...
```
- [ ] 1. Set type (`combobox`) and unique id
...
## Field guide (primary type: `combobox`)
...
| `items` | no | Options shown in the list (strings). |
...
2. Use **stable, descriptive ids** (`combobox-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `combobox` |

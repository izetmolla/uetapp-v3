# Button Group — JSON reference

TypeScript: `renders/button-group/types.ts`.

Registered manifest types: `button-group`, `button-group-separator`, `button-group-text`.
...
## ButtonGroupItem (`type: "button-group"`)

Extends `BaseLayoutItem`.

| Field | Type | Description |
|-------|------|-------------|
| `type` | `"button-group"` | Discriminant |
| `id` | string | Unique identifier |
| `children` | — |  |

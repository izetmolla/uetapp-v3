---
name: generate-sonner-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `sonner`. Use when
  building layout JSON, MCP layout tools, or adding Sonner items to
  LayoutBuilder trees.
---

# Generate Sonner layout JSON
...
```
- [ ] 1. Set type (`sonner`) and unique id
...
## Field guide (primary type: `sonner`)

| Field | Required | Notes |
|-------|----------|-------|
| `type` | yes | `"sonner"` |
| `id` | yes | Unique in tree |
| `position` | no | See types.ts |
...
2. Use **stable, descriptive ids** (`sonner-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `sonner` |

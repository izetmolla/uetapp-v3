---
name: generate-scroll-area-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `scroll-area`. Use when
  building layout JSON, MCP layout tools, or adding Scroll Area items to
  LayoutBuilder trees.
---

# Generate Scroll Area layout JSON
...
```
- [ ] 1. Set type (`scroll-area`) and unique id
...
## Field guide (primary type: `scroll-area`)

| Field | Required | Notes |
|-------|----------|-------|
| `type` | yes | `"scroll-area"` |
...
2. Use **stable, descriptive ids** (`scroll-area-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `scroll-area` |

---
name: generate-multi-select-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `multi-select`. Use when
  building layout JSON, MCP layout tools, or adding Multi Select items to
  LayoutBuilder trees.
---

# Generate Multi Select layout JSON
...
```
- [ ] 1. Set type (`multi-select`) and unique id
...
## Field guide (primary type: `multi-select`)

| Field | Required | Notes |
|-------|----------|-------|
| `type` | yes | `"multi-select"` |
...
2. Use **stable, descriptive ids** (`multi-select-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `multi-select` |

---
name: generate-pagination-json
description: >-
  Generates and validates Flowtrove layout builder JSON for `pagination`. Use when
  building layout JSON, MCP layout tools, or adding Pagination items to
  LayoutBuilder trees.
---

# Generate Pagination layout JSON
...
```
- [ ] 1. Set type (`pagination`) and unique id
...
## Field guide (primary type: `pagination`)
...
2. Use **stable, descriptive ids** (`pagination-save`, not `x1`).
...
| Issue | Fix |
|-------|-----|
| Wrong `type` string | Use exactly: `pagination` |

...
export function getActionsColumn(
    setRowAction?: React.Dispatch<React.SetStateAction<DataTableRowAction<Department> | null>>,
...
export function prependColumns(): Array<{ id: string } & Partial<ColumnDef<Department>>> {
    return [
        withNameColumnLayout<Department>({
            id: "name",
            cell: ({ row }) => (
                <EntityNameCell
                    ...
                    subtitle={row.original?.faculty_name ?? undefined}
                />
            ),
        }),
    ];
